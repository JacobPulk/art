...
- The final two function values nudge that color back toward a target color. One chooses the target color from the palette; the other determines how closely to approach.

These 8 functions are generated by applying an **artist**, which is a stochastic algorithm for generating functions. The functions built by an artist are composed of dozens or hundreds of atomic functions (like `X`, `Y`, `POW`, and `SIN`). Atomic functions have various arguments. For example, `POW` takes a variable base (an output of another atomic function) and raises it to a constant exponent.  
...
When there are multiple panels in an image, they are generated identically, apart from the use of a different [seed](https://en.wikipedia.org/wiki/Random_seed) for the random number generator at a select few random parts of the process.
