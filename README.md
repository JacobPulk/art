...
The program can produce a wide diversity of images considering its "simplicity". Using it is analogous to bushwacking through a jungle of unpleasant functions or images to find areas with fruitful aesthetics.

Essentially, an image is drawn in one pass from a single continuous function of the XY plane.

There is no "composition" beforehand, no shapes, no sprites, no layers, and no glitches or other effects applied afterwards.
