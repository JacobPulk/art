...
These 6 functions are generated by applying a **scheme**, which is a stochastic algorithm for generating functions composed of dozens or hundreds of atomic functions (like `X`, `Y`, `POW`, and `SIN`).  
These functions have various arguments. For example, `POW` takes a variable base (an output of another atomic function) and raises it to a constant exponent.
