...
The program can produce a wide diversity of images. Using it could be compared to bushwacking through a jungle of unpleasant outputs to find areas with fruitful aesthetics.

The program is intended to be "simple", as far as following a number of constraints.

Essentially, an image is drawn in one pass from one continuous function of the XY plane. There is no "composition" beforehand, no shapes, no sprites, no layers, and no glitches or other effects applied afterwards. More detail is at the [end of this page](#-How-it-does-not-work).
