...
Essentially, an image is drawn in one pass from one continuous function of the XY plane. There is no geometric "composition" beforehand, no shapes, no layers, and no glitches or other effects applied afterwards. More detail about the constraints is at the [end of this page](#how-it-does-not-work).
...
- Each theme is made for a particular scheme and only works with that scheme. Themes' full names make this clear: the theme stored as `belize_gamma.json` is for the scheme `belize.json`. Reusing theme suffixes across different schemes is fine. For example, `angola_gamma.json` is an unrelated theme for the scheme `angola.json`. I use this Greek alphabet convention where `alpha` is the first saved theme for that scheme, `beta` is the second, etc., but you do not need to do this.
...
- In the `data` folder are several subfolders. Look at the `data\images` folder to see the saved (re-renderable) images. Look at the `data\themes` folder to see the saved (usable) themes. Look at the `data\schemes` subfolder to see the saved (usable) schemes.

- You can rename these files freely, as long as you maintain simple naming conventions: If you rename an image in `data\images`, rename the corresponding ptych in `data\ptychs`. If you rename a scheme in `data\schemes`, rename the corresponding themes in `data\themes`.
...
`butterfly alpha` will generate and show a new image generated by the **scheme** called "butterfly" and its **theme** called "alpha".

`..` will generate a new image from a random saved **scheme** and **theme**.

`.` will generate a new image from a random saved **theme** for the last used **scheme**.
...
`!!` will find a new **theme** for a random saved **scheme**. (Note: You will notice that this search sometimes investigates multiple schemes before finding a theme. This is intentional - see [Settings](#5-settings).)
...
Panels in a **ptych** are similar but different. This is _not_ accomplished by generating one panel and tweaking it several times. Even if such a method was implemented carefully enough that modified panels were indistinguishable from "properly" generated ones, it would still violate the spirit of the program by depending on post-processing. Instead, all panels are generated by the exact same process, with the random number generator (Python `random` module) seeded differently for (randomly) selected, brief intervals within the generating methods. At initialization, all panels are assigned the same (random) sequence of seeds for their reseed points, after which each panel has new seed values assigned to a small random subset of its reseed points. Reseed points are carefully arranged in the code so they are each reached exactly once during the creation of a panel. This is necessary for the numbers generated at the *n*th point in one panel to be being used for the same purpose as the numbers generated at the *n*th point in another panel.
