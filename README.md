...
While randomness influences the palette and function, there is no randomness applied once rendering starts; the image is a deterministic "output" of those "inputs". Even applying the `RAND` component function is deterministic in that the values are generated by the same seed to the `random` module (arbitrarily, 0)—it is one unchanging function that looks random.  
