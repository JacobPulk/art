...
- A scheme, theme, and image are analogous to a make, model, and road trip. A scheme has its own themes—a given theme will only work for one particular scheme. Together, they specify a stochastic (random number-influenced) system for creating images. In other words, an image is created by one scheme, one theme, and some randomness.

- Schemes and themes are saved as `.json` files. The program comes with many "core" schemes and themes. These were not made or manipulated by hand, but randomly generated by this program and chosen/saved by me. You can use these and/or find your own. Keep in mind that new images from old schemes/themes are still new.
...
- In the `data` folder are the `images`, `ptychs`, `themes`, and `schemes` folders. Look at the former two to see the saved/re-renderable images, and the latter two to see the saved/usable **schemes** and **themes**. You can rename these files freely, as long as you maintain the same obvious naming conventions (**ptychs** and images have corresponding filenames; schemes and their themes have corresponding filenames).
