...
In summary, images are composed by procedure and randomness, and are curated by users. Those procedures are composed by procedure and randomness, and are curated by users.
...
**In summary**, an image is defined by a palette and 7 mathematical functions. The palette is generated more or less independently.

The functions are single-valued [almost everywhere](https://en.wikipedia.org/wiki/Almost_everywhere) [continuous](https://en.wikipedia.org/wiki/Continuous_function) functions of the XY plane.

The image is drawn in one pass, by applying the palette and functions to each pixel's coordinates to arrive at a color for it.

**At each pixel**, one function value determines the number of iterations the rest of the functions will undergo.
