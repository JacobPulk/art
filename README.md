...
Rendering an image, from the function that describes it, must be deterministic. (Even applying the `RAND` component function is deterministic in that the values are always generated by the same seed to the `random` module. It is one unchanging function that looks random.)
